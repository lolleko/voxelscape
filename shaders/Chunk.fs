...
void main() {
    vec3 norm = normalize(inp.normal);

    vec3 viewDir = normalize(viewPos - inp.worldPosition);
    ...
    vec3 rayStart = inp.worldPosition;

    vec3 lightDir = normalize(lightPos - inp.worldPosition);

    float shadowFactor = enableShadows ? raymarch(rayStart, lightDir) : 1.0;

    // ambient
    float ambientStrength = 0.2;
    float occ = 1.0; // TODO enableAO ? calcAO(inp.worldPosition, norm) : 1.0;
    ...
    vec3 result = (ambient + shadowFactor * (diffuse + specular)) * inp.color;

    outColor = vec4(result, 1.0);

    //outColor = vec4(occ);

    //outColor = vec4(vec3(inp.worldPosition.x + worldSizeHalf.x, inp.worldPosition.y + worldSizeHalf.y, inp.worldPosition.z + worldSizeHalf.z) / worldSize, 1.0);

    //outColor = vec4((inp.worldPosition + worldSize / 2u) / worldSize, 1);

    //outColor = vec4(map(inp.worldPosition + vec3(0, 24, 0)) / 255, 0, 0, 1);

    //outColor = vec4(map(inp.worldPosition) / 255, 0, 0, 1);
}